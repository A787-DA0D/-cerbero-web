...
export default function LoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      // 3) Salviamo il token di sessione (per ora localStorage, poi lo potremo migliorare)
      localStorage.setItem("cerbero_session", data.sessionToken);

      // 4) Redirect alla dashboard interna
      window.location.href = "/dashboard";
      // se la tua dashboard è /app/dashboard, usa:
  ...
  return (
    <main className="min-h-screen bg-[#020617] bg-gradient-to-b from-[#020617] via-[#020617] to-[#f1f5f9] flex items-center justify-center px-4">
      <div className="max-w-5xl w-full mx-auto">
        <div className="relative mx-auto flex flex-col md:flex-row gap-10 rounded-[32px] bg-black/40 border border-white/5 px-8 py-10 md:px-12 md:py-12 shadow-[0_40px_120px_rgba(0,0,0,0.7)] backdrop-blur-xl">
          {/* Colonna sinistra: testo / highlights */}
          <div className="flex-1 space-y-6">
            ...
            <div>
              ...
              <p className="mt-3 text-sm md:text-base text-slate-300/90 leading-relaxed">
                Entra nella tua dashboard per controllare saldo, pilota e
                operatività. Login{" "}
            ...
            <ul className="space-y-2 text-sm text-slate-200/90">
              ...
              <li className="flex items-start gap-2">
                <span className="mt-[3px] h-1.5 w-1.5 rounded-full bg-emerald-400" />
                <span>
                  Nessuna password da ricordare, solo la tua email personale.
                </span>
              </li>
            </ul>
          ...
          <div className="flex-1">
            <div className="mx-auto max-w-md rounded-3xl bg-slate-950/80 border border-white/10 px-6 py-7 md:px-7 md:py-8 shadow-[0_30px_80px_rgba(0,0,0,0.9)]">
              ...
              <form className="space-y-4" onSubmit={handleLogin}>
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2.5 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/70 focus:border-emerald-500/70"
                  />
